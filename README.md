...
- `public/content.json` — **NEWS / WORKS / PROFILE / LINKS の正本**(管理画面から編集するのもこの内容)
  ...
  - 各作品の `badgeType` = `award`(受賞: 金の金属ラベル・光が流れる+トロフィー+光の粒) / `honor`(ノミネート・選考通過: **銀の金属ラベル**+✦。金と同じ作りで輝きの強さだけ落とす) / `default`(参加・自主制作: 暗いピル)。未指定ならバッジ文が 🏆 で始まるときだけ award 扱い。🏆 はアイコンで示すので文字には入れない
...
- PROFILE の年表(2026-09-11): **大きなプロットだけを置く方針**(りあさん)。受賞・選出・初挑戦など節目のみで、細かいお知らせは NEWS 側。受賞回は `award: true` とタイトル末尾の 🏆(既存の書き方に合わせる)。2026.09 に MIDNIGHT MART を追加
